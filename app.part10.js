...
const btnDownloadTrips = document.getElementById("btnDownloadTrips");
...
function setAuthUI(signedIn, note) {
  ...
  if (btnDownloadTrips) btnDownloadTrips.classList.toggle("disabled", !signedIn);
...
// Download all of the signed-in user's pickup trips as a ZIP (CSV + JSON) so
// they can keep an external backup that survives a server/database reset.
async function downloadMyTrips() {
  if (!requireCommunityToken("back up your trips")) return;
  const btn = btnDownloadTrips;
  ...
  try {
    ...
    const res = await fetch(`${apiBase}/me/pickups/export`, {
    ...
    // Prefer the server's filename; fall back to a dated default.
    let filename = `pickup-trips-${new Date().toISOString().slice(0, 10)}.zip`;
    ...
    alert(err?.detail || err?.message || "Could not back up your trips. Please try again.");
...
if (btnDownloadTrips) {
  btnDownloadTrips.addEventListener("pointerdown", (e) => e.stopPropagation());
  btnDownloadTrips.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    void downloadMyTrips();
  });
}
