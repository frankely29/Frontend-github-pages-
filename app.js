...
// STRICT: reads Railway only using /timeline + /frame/{idx}
// Slider starts at closest NYC current time window (week wrap)
...
const polyLayer = L.geoJSON(null, {
  ...
  style: (f) => {
    ...
    return st || { color: "#999", opacity: 0, weight: 0, fillColor: "#999", fillOpacity: 0.0 };
  },
...
function renderFrame(frame) {
  document.getElementById("timeLabel").textContent = formatTimeLabelNYC(frame.time);
...
async function main() {
  ...
  if (timeline.length > 0) {
    ...
    document.getElementById("timeLabel").textContent = "No timeline. Run /generate on Railway.";
  }
}
