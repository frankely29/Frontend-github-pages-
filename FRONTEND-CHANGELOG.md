...
- `nycHourAndDay()` now also returns the NYC `ymd`/`md`; new `closureReason()` + `mdInRange()` gate `dimForHotspot`/`primeForHotspot`; `sanitizeCalendar()` defensively parses the new field.
