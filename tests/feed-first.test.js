...
test('the menu button is there whatever is in the sheet', () => {
  ...
  // And while the menu is down, because the panel is translucent and a button
  // left under it shows through as a ghost.
  assert.ok(/body\.feed-first\.shell-menu-open\.shell-screen-open \.shellMenuBtn/.test(CSS),
    'the button stays under the open menu');
});

test('the menu meets the feed instead of cutting the screen in half', () => {
  /* It was a left drawer: min(306px, 86vw) of slab with a hard vertical edge
   * and a sliver of map showing past it. Now it drops from the top and its
   * bottom edge is the same number as the sheet's top edge -- var(--tj-split)
   * -- so the two meet in a seam, and if the sheet is dragged the seam follows
   * it, because neither side owns the number. */
  ...
  // It has to slide from the right edge too: app-shell's translateX would
  // shove it sideways off a panel that now spans the screen.
...
test('a short wide box holds a grid, not a list', () => {
  /* Eleven rows 100px apart down a column is what a 306x-tall drawer wants.
   * In the top panel that same list would need a scroll to show Admin, so the
   * items become tiles: four across, eleven of them, all reachable at once. */
  const body = CSS.match(/body\.feed-first \.shellMenuBody\s*\{([^}]*)\}/s);
  assert.ok(body, 'no menu body rule');
  assert.ok(/display:\s*grid/.test(body[1]), 'the menu body is still a list');
  assert.ok(/grid-template-columns:\s*repeat\(4,/.test(body[1]),
    'the grid is not four across');
  // Centred rather than stretched: rows sharing out the room turned the tiles
  // into 100x149 slabs at the minimised detent, and hugging the top left a
  // quarter of the panel as empty glass under them.
  assert.ok(/align-content:\s*center/.test(body[1]), 'the block does not sit in the middle');

  // Four group headings among eleven tiles would spend a third of the grid
  // saying what the tiles already say. The order they impose is kept.
  assert.ok(/body\.feed-first \.shellMenuBody \.shellGroup\s*\{[^}]*display:\s*none/s.test(CSS),
    'the group headings still take a row each');
  assert.ok(/body\.feed-first \.shellMenuBody \.shellItemSub\s*\{[^}]*display:\s*none/s.test(CSS),
    'the subtitles still have to fit in a 99px tile');

  const item = CSS.match(/body\.feed-first \.shellMenuBody \.shellItem\s*\{([^}]*)\}/s);
  assert.ok(item, 'no tile rule');
  assert.ok(/flex-direction:\s*column/.test(item[1]), 'the tile is still a row');
  // The corner marks come out of the tile's own column so they cost it no
  // height -- a 78px tile has none to give.
  assert.ok(/position:\s*relative/.test(item[1]), 'the corner marks have nothing to anchor to');
  assert.ok(/body\.feed-first \.shellMenuBody \.shellItemHere\s*\{[^}]*position:\s*absolute/s.test(CSS),
    'the HERE badge still sits in the flow');
  assert.ok(/body\.feed-first \.shellMenuBody \.shellItemLock\s*\{[^}]*position:\s*absolute/s.test(CSS),
    'the padlock still sits in the flow');
});

test('where you are is the filled tile, in both palettes', () => {
  // The dock already says "here" by filling its current button with ink. A
  // translucent wash on a translucent panel said it much more quietly.
  const on = CSS.match(/body\.feed-first \.shellMenuBody \.shellItem\.on\s*\{([^}]*)\}/s);
  assert.ok(on && /background:\s*#101622/.test(on[1]), 'the current tile is not filled');
  assert.ok(/body\.feed-first \.shellMenuBody \.shellItem\.on \.shellItemTitle\s*\{[^}]*color:\s*#fff/s.test(CSS),
    'ink on ink');

  // Inverted at night, or the fill is near-black on near-black and the whole
  // point of it disappears.
  const night = CSS.match(/body\.night\.feed-first \.shellMenuBody \.shellItem\.on\s*\{([^}]*)\}/s);
  assert.ok(night && /background:\s*#eef2f8/.test(night[1]), 'the night fill is still dark');
  assert.ok(/body\.night\.feed-first \.shellMenuBody \.shellItem\.on \.shellItemTitle,[\s\S]{0,120}color:\s*#101622/.test(CSS),
    'white on white at night');
  // And the badge sitting on that fill flips with it.
  assert.ok(/body\.night\.feed-first \.shellMenuBody \.shellItem\.on \.shellItemHere/.test(CSS),
    'the HERE badge is light-on-light on the night fill');
});
