...
(function() {
  ...
  function mapIdentityApplyOrbitStyleToSlot(slot, orbitMeta = null, zoomValue = null) {
    if (!slot) return;
    ...
    if (Number.isFinite(orbitMeta?.slotIndex)) slot.dataset.orbitIndex = String(orbitMeta.slotIndex);
  ...
  function mapIdentityApplySelfOrbit(orbitMeta) {
    ...
    if (!slot) return;
    mapIdentityApplyOrbitStyleToSlot(slot, orbitMeta, Number.isFinite(window?.map?.getZoom?.()) ? window.map.getZoom() : null);
  }
