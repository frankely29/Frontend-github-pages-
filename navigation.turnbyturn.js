(function () {
  ...
  const state = {
    ...
    hotspotFillOpacityOriginal: new Map(),
  };
  ...
  function setHotspotsDimmed(dimmed) {
    const map = state.map;
    if (!map?.getLayer?.("zones-fill")) return;
    if (dimmed) {
      if (!state.hotspotFillOpacityOriginal.has("zones-fill")) {
        state.hotspotFillOpacityOriginal.set("zones-fill", map.getPaintProperty("zones-fill", "fill-opacity"));
      }
      map.setPaintProperty("zones-fill", "fill-opacity", 0.2);
      return;
    }

    if (state.hotspotFillOpacityOriginal.has("zones-fill")) {
      const original = state.hotspotFillOpacityOriginal.get("zones-fill");
      map.setPaintProperty("zones-fill", "fill-opacity", original == null ? 1 : original);
      state.hotspotFillOpacityOriginal.delete("zones-fill");
    }
  }
